...
#    H = []
#    for datapoint in X:
#        c_list = [np.dot(theta_j, datapoint) / temp_parameter for theta_j in theta]
#        c = max(c_list)
#        entries = [math.exp(exponent - c) for exponent in c_list]
#        vector = np.array([entry for entry in entries])
#        h = (1 / sum(entries)) * vector
#        H.append(list(h))
...
#            tmp_mat.append(Y[i] == j)
#        mat.append(tmp_mat)
#    mat = np.array(mat).T
#    probabilities = compute_probabilities(X, theta, temp_parameter)
#    cost_term = mat * np.log(probabilities)    
#    squared_theta = theta ** 2
#    regulation_term = (lambda_factor / 2) * squared_theta
#    cost_term *= -1 / X.shape[0]
#    return np.sum(cost_term) + np.sum(regulation_term)
#   Vectorized solution
    N = X.shape[0]
    probabilities = compute_probabilities(X, theta, temp_parameter)
    ...
    return non_regulizing_cost + regulizing_cost ## kolla om det finns lösning utan np.choose
