...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    return max(1 - label * (np.dot(feature_vector, theta) + theta_0), 0)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for index in range(len(feature_matrix)):
        sum_hl += hinge_loss_single(feature_matrix[index], labels[index], 
                                    theta, theta_0)
    return sum_hl / len(feature_matrix)        
...
def perceptron_single_step_update(feature_vector, label, current_theta, 
    ...
    completed.
    """
    if label * (np.dot(feature_vector, current_theta) + current_theta_0) <= 1e-7:
...
def extract_bow_feature_vectors(reviews, dictionary):
    """
    Args:
        reviews - list of string reviews
        dictionary - dictionary of words as given by bag_of_words
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                feature_matrix[i, dictionary[word]] = 1 # += 1 changes binary update to counts 
